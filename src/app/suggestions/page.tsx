...
export default function SuggestionsPage() {
  ...
  // Featured Burgueriza Lounge Spot
  ...
  return (
    ...
    >
      ...
      {/* DISMISSIBLE HERO FEATURED SPOT BAR AT TOP */}
      {!isHeroSpotDismissed && (
        <section className="w-full bg-[#1E1412] text-white border-b border-stone-800 py-3 px-4 shadow-md">
          <div className="site-container flex items-center justify-between gap-3 text-xs">
            <div className="flex items-center gap-3 min-w-0">
              <span className="bg-[#F59E0B] text-black font-mono font-bold px-2 py-0.5 rounded text-[10px] uppercase shrink-0">
                Featured Spot
              </span>
              <span className="font-syne font-bold truncate">Burgueriza Lounge</span>
              <span className="text-stone-400 text-[11px] hidden sm:inline truncate">
                • Bole Atlas • Gourmet Flame-Grilled Cheese Burgers
              </span>
            </div>
            ...
            <div className="flex items-center gap-2 shrink-0">
              <button
                ...
                className="text-[#F59E0B] hover:underline font-mono font-bold text-[11px]"
              >
                Inspect →
              </button>
              <button
      ...
      {/* TWO MAIN REVIEW ROWS AT THE TOP */}
      <main className="site-container py-8 flex flex-col gap-12 flex-1">
        ...
        <section className="flex flex-col gap-4">
          ...
          {/* Row 1 InfiniteSlider Smooth Marquee */}
          <AutoReviewMarquee
        ...
        <section className="flex flex-col gap-4">
          ...
          {/* Row 2 InfiniteSlider Smooth Marquee */}
          <AutoReviewMarquee
            posts={foodRowPosts}
            onSelectPost={(post) => setModalPost(post)}
          />
      ...
      {/* HEADER TITLE, SUBTEXT & NOMINATION BUTTON POSITIONED AT THE BOTTOM */}
      <section
        className="w-full py-12 border-t border-b transition-colors"
      ...
      >
        <div className="site-container flex flex-col items-center text-center gap-5">
          ...
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full text-xs font-mono font-bold uppercase tracking-wider bg-[#F59E0B]/10 text-[#F59E0B] border border-[#F59E0B]/20">
            ...
            <span>{lang === 'AM' ? 'ወደ የት እንሂድ? የመመገቢያ ጥቆማዎች' : 'Where To Go — Spot Recommendations'}</span>
          </div>

          <h2 className="font-syne font-black text-3xl sm:text-4xl text-[var(--text-primary)]">
          ...
          <button
            ...
            className="mt-2 button-primary px-6 py-3 rounded-2xl text-xs font-label uppercase tracking-wider text-white font-bold flex items-center gap-2.5 shadow-lg cursor-pointer hover:scale-105 transition-transform"
          >
      ...
      {/* COLLAPSIBLE NOMINATION FORM DRAWER SECTION AT BOTTOM */}
      {isFormOpen && (
        <section className="w-full bg-[var(--bg-app)] border-b border-[var(--border-subtle)] py-8 animate-fadeIn">
          <div className="site-container max-w-2xl mx-auto">
            {submitted ? (
              <div className="p-8 rounded-2xl bg-[var(--bg-surface)] border border-emerald-500/30 text-center flex flex-col items-center gap-4 shadow-lg">
              ...
              <form
                ...
                className="p-6 sm:p-8 rounded-2xl bg-[var(--bg-surface)] border border-[var(--border-subtle)] shadow-md flex flex-col gap-5"
              >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="flex flex-col gap-1.5">
                    ...
                    <input
                      ...
                      className="px-3.5 py-2 rounded-xl bg-[var(--bg-app)] border border-[var(--border-subtle)] text-xs text-[var(--text-primary)] focus:outline-none focus:border-[#A81D1D]"
                    />
                  </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  ...
                  <div className="flex flex-col gap-1.5">
                    ...
                    <select
                      ...
                      className="px-3 py-2 rounded-xl bg-[var(--bg-app)] border border-[var(--border-subtle)] text-xs text-[var(--text-primary)] focus:outline-none focus:border-[#A81D1D]"
                    >
                ...
                <div className="flex flex-col gap-1.5">
                  ...
                  <textarea
                    ...
                    className="px-3.5 py-2 rounded-xl bg-[var(--bg-app)] border border-[var(--border-subtle)] text-xs text-[var(--text-primary)] focus:outline-none focus:border-[#A81D1D] resize-none"
                  />
                </div>
